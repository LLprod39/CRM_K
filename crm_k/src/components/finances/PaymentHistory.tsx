...
import { Calendar, Clock, User, CheckCircle, ChevronRight, Wallet, DollarSign } from 'lucide-react'
...
export default function PaymentHistory() {
  ...
  const fetchPaymentHistory = async () => {
    try {
      ...
      const response = await fetch('/api/finances/payment-history', {
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Ошибка при загрузке истории платежей:', response.status, response.statusText)
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-500">
            Прошедшие и оплаченные занятия
          </p>
        </div>
        <div className="p-6">
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border">
        ...
        <div className="p-6 text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">Нет оплаченных занятий</h3>
          <p className="mt-1 text-sm text-gray-500">
            Прошедшие и оплаченные занятия появятся здесь
          </p>
        </div>
      </div>
    )
  ...
  const displayedStudents = paymentHistory.slice(0, maxVisibleItems)
  const hasMoreStudents = paymentHistory.length > maxVisibleItems
        ...
        <div className="flex-1 overflow-hidden">
          <div className="divide-y divide-gray-200 h-full">
            {displayedStudents.map((studentData) => (
              <div key={studentData.student.id} className="p-4 flex-shrink-0">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
        ...
        {hasMoreStudents && (
          <div className="px-6 py-4 border-t border-gray-200 flex-shrink-0">
            ...
            >
              ...
              Показать всех учеников ({paymentHistory.length})
            </button>
          </div>
        )}
      </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {paymentHistory.length} {paymentHistory.length === 1 ? 'ученик' : 'учеников'} с оплаченными занятиями
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {paymentHistory.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h4 className="text-lg font-medium text-gray-900 mb-2">Нет оплаченных занятий</h4>
                  <p className="text-gray-500">Прошедшие и оплаченные занятия появятся здесь</p>
                </div>
              ) : (
                <div className="grid gap-4">
                  {paymentHistory.map((studentData, index) => (
                    <div key={studentData.student.id} className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-200 hover:border-green-200">
                      <div className="flex items-center justify-between">
